...
INT WINAPI GetMD5FileHash(PCHAR sFilepath, LPSTR sHash) {
	...
	if (INVALID_HANDLE_VALUE == hFile) {
		DisplayErrorEx((LPSTR)"iCrypto:cHash", (LPSTR)"CreateFile");
	...
	if (!CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)) {
		DisplayErrorEx((LPSTR)"iCrypto:cHash", (LPSTR)"CryptAcquireContext");
		CloseHandle(hFile);
	...
	if (!CryptCreateHash(hProv, CALG_MD5, 0, 0, &hHash)) {
		DisplayErrorEx((LPSTR)"iCrypto:cHash", (LPSTR)"CryptCreateHash");
	...
	if (!hFile) {
		DisplayErrorEx((LPSTR)"iCrypto:cHash", (LPSTR)"GetFileSize");
		CloseHandle(hFile);
	...
	if (!rgbFile) {
		DisplayErrorEx((LPSTR)"iCrypto:cHash", (LPSTR)"LocalAlloc");
		CloseHandle(hFile);
	...
	while (bResult = ReadFile(hFile, rgbFile, hSize, &cbRead, NULL)) {
		...
		if (!CryptHashData(hHash, rgbFile, cbRead, 0)) {
			DisplayErrorEx((LPSTR)"iCrypto:cHash", (LPSTR)"CryptHashData");
	...
	if (!bResult) {
		DisplayErrorEx((LPSTR)"iCrypto:cHash", (LPSTR)"ReadFile");
	...
	if (CryptGetHashParam(hHash, HP_HASHVAL, rgbHash, &cbHash, 0))
	{
		...
		DisplayErrorEx((LPSTR)"iCrypto:cHash", (LPSTR)"CryptGetHashParam");
